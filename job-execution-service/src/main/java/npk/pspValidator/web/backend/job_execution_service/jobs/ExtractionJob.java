...
public class ExtractionJob extends Job {
    ...
    public void run() {
        new Thread(() -> {
            ...
            File extractionLogFile = new File(Config.instanceOf().getValidationsWorkingDir(), validationId + File.separator + "extraction.log");
            PrintStream out = null;
            try {
                out = new PrintStream(extractionLogFile);
                out.println("Extraction log for " + validationId);
            } catch (IOException e) {
                //TODO: ulozit chybovou hlasku do zaznamu validace
    ...
    private void extractZipFile(String validationId, PrintStream out) throws IOException {
        File zipFile = new File(Config.instanceOf().getValidationsWorkingDir(), validationId + File.separator + "uploaded.zip");
        File destDir = new File(Config.instanceOf().getValidationsWorkingDir(), validationId + File.separator + "extracted");
